...
unsafe fn cast_codegen<'a>(p: &mut Codegen) -> &'a mut Codegen<'a> {
    unsafe { &mut *(p as *mut Codegen as *mut Codegen<'a>) }
}
...
#[inline(always)]
fn emit_this_expression_impl<'a>(expr: &ThisExpression, p: &mut Codegen<'a>, _ctx: Context) {
    p.print_space_before_identifier();
...
#[inline(always)]
fn emit_super_impl<'a>(expr: &Super, p: &mut Codegen<'a>, _ctx: Context) {
    p.print_space_before_identifier();
    p.add_source_mapping(expr.span);
...
impl<'a, 'gc> StatementEmitter<'a, 'gc> {
    #[inline(always)]
...
fn emit_function_body_impl<'a>(body: &FunctionBody<'a>, p: &mut Codegen<'a>, ctx: Context) {
    ...
    if single_line {
        if let Some(key) = trailing_comments_key {
            if let Some(block) = p.peek_comments(key) {
                single_line =
                    block.iter().all(|c| !c.preceded_by_newline() && !c.followed_by_newline());
            }
        }
    }
    p.print_curly_braces(body.span, single_line, |p| {
        ...
        if let Some(key) = trailing_comments_key {
            if let Some(comments) = p.take_comments(key) {
...
fn emit_catch_clause_impl<'a>(clause: &CatchClause<'a>, p: &mut Codegen<'a>, ctx: Context) {
    ...
    p.print_soft_space();
    emit_block_statement_impl(&clause.body, p, ctx);
}
...
#[inline(always)]
fn emit_jsx_empty_expression_impl<'a>(
    expr: &JSXEmptyExpression,
    p: &mut Codegen<'a>,
    _ctx: Context,
) {
...
#[inline(always)]
fn emit_jsx_opening_fragment_impl<'a>(
    fragment: &JSXOpeningFragment,
    p: &mut Codegen<'a>,
    _ctx: Context,
) {
...
#[inline(always)]
fn emit_jsx_closing_fragment_impl<'a>(
    fragment: &JSXClosingFragment,
    p: &mut Codegen<'a>,
    _ctx: Context,
) {
...
#[inline(always)]
fn emit_jsx_text_impl<'a>(text: &JSXText<'a>, p: &mut Codegen<'a>, _ctx: Context) {
    p.add_source_mapping(text.span);
    p.print_str(text.value.as_str());
}

#[inline(always)]
fn emit_jsx_spread_child_impl<'a>(child: &JSXSpreadChild<'a>, p: &mut Codegen<'a>, _ctx: Context) {
    p.print_str("{...");
...
fn emit_ts_type_impl<'a>(ty: &TSType<'a>, p: &mut Codegen<'a>, ctx: Context) {
    ...
    match ty {
        ...
        TSType::TSUnknownKeyword(_) => p.print_str("unknown"),
        ...
        TSType::JSDocUnknownType(_) => p.print_str("unknown"),
    }
}
...
fn emit_ts_template_literal_type_impl<'a>(
    ...
    for (index, item) in ty.quasis.iter().enumerate() {
        if index != 0 {
            if let Some(types) = ty.types.get(index - 1) {
                p.print_str("${");
                types.print(p, ctx);
                p.print_str("}");
            }
